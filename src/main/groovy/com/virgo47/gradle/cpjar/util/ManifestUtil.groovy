package com.virgo47.gradle.cpjar.util
...
/**
 * Utility class to prepare classpath jar from file collection
...
class ManifestUtil {
    ...
    /**
     * Utility API to create classpath JAR from file collection
     ...
     */
    public static void prepareClasspathJar(def jarFile, def fileCollection) {
        if (!fileCollection) {
            return
        }
        ...
        createClasspathJar(jarFile, manifestWithClasspath(classPathString))
    }

    private static def createClasspathJar(def jarFile, def manifest = null) throws IOException {
        def jarOutputStream
